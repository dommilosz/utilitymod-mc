...
public class try_jump {
    ...
    public static void executePacketsSync() {
        ...
        for (int i = 0; i < TJpackets.size(); i++) {
            ...
            String msg = "";
            if (i + 1 == TJpackets.size())
                msg = Color.BLUE + "Sending packets: [" + Color.YELLOW + "DONE" + Color.BLUE + "]";
            else
                msg = Color.BLUE + "Sending packets: [" + Color.YELLOW + (i + 1) + "/" + TJpackets.size() + Color.BLUE + "] - " + Color.YELLOW + toGotime(i) + "ms";
